...
import { fetchUsers } from '@/services/redux_slices/userSlice';
...
const Users = () => {
  ...
  return (
    <div className="flex flex-col justify-between min-h-[100vh]">
      ...
      {!users.loading && users.error && <Alerts type='error' message={users.error} />}
      <Navbar />
          ...
          <tbody>
            {usersData.map(user => (
              <tr key={user._id} className="h-12 hover:bg-tertiary hover:bg-opacity-10">
                ...
                <td className="pt-1 pl-1 border-r-primary border-r md:border-0 ">{user.photo ? user.photo : '_'}</td>
