  ...
  return (
    <div className="relative flex flex-row items-center justify-between px-[48px] py-2 h-[10vh] bg-primary">
      <div className="flex flex-row items-center justify-center">
        <Image src='/whoget_white.png' width={100} height={100} alt="whoget logo" className="h-auto w-auto" />
      </div>
      <div className="hidden md:flex flex-row items-center justify-center space-x-4 text-white">
        ...
        <div onClick={() => { setShowMenu(prevState => !prevState) }} className={`${asPath==="/users" && "ring-1"} ring-white py-1 px-2 w-full flex items-end hover:underline duration-300 hover:cursor-pointer`}><Link href="/users">Users</Link> </div>
        <div onClick={() => { setShowMenu(prevState => !prevState) }} className={`${asPath==="/asks" && "ring-1"} ring-white py-1 px-2 w-full flex items-end hover:underline duration-300 hover:cursor-pointer`}><Link href="/asks">Asks</Link> </div>
      </div>
      <div className="hidden md:flex">
        <div onClick={() => { }} className="px-3 bg-white py-1 text-primary self-center hover:opacity-70 duration-200 hover:cursor-pointer rounded-md"><Link href="/authenticate">Log out</Link></div>
      </div>
      ...
      {showMenu &&
        <div className="absolute top-[15vh] left-0 w-3/4 h-[100vh] flex md:hidden flex-col items-start justify-start px-7 text-white bg-primary divide-y">
          ...
          <div onClick={() => { setShowMenu(prevState => !prevState) }} className="w-full flex items-end py-2 hover:px-2 hover:bg-white hover:text-primary duration-200 hover:cursor-pointer"><Link href="/authenticate">Log out</Link> </div>
        </div>
      }
    </div>
  )
